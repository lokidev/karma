...
* The import-data.sh script also uses bcp to bulk import the data found in the Karma.csv file.
...
The setup.sql script will create a new database called `DemoData` and a table called `Karma` in the default `dbo` schema.  This bcp command will import the data contained in the source code file Karma.csv.
...
USE DemoData;
GO
CREATE TABLE Karma (ID int, KarmaName nvarchar(max));
GO
```

### Karma.csv

This CSV data file contains some karma data to populate the Karma table.  It has two columns - ID and KarmaName separated by a comma.  The bcp command in the import-data.sh script uses this file to import the data into the Karma table created by the setup.sql script file.
...
```node
sqlreq = new request("SELECT * FROM Karma FOR JSON AUTO", function(err, rowCount) {
```
